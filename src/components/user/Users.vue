...
<!--            删除按钮type="danger"-图标显示红色-->
            <el-button type="danger"
                       icon="el-icon-delete"
                       size="mini"
                       @click="removeUserById(scope.row.id)"></el-button>
<!--            分配角色按钮type="warning"-图标显示黄色-->
...
<!--      分页区域-->
<!--      :page-size="qureyInfo.pagesize当前每页显示多少条数据-->
      <el-pagination
...
<script>
  export default {
    ...
    methods: {
      getUserList() {
        this.axios.get('users', { params: this.qureyInfo}).then(res => {
          // console.log(res)
          // this.user = res.data
          if(res.status !== 200) {
      ...
      addUser() {
        this.$refs.addFormRef.validate(valid => {
          // 如果校验失败，则直接return出去，不走添加的业务逻辑了
          if(!valid) return
          // 如果没有return出去，证明表单验证成功，可以发起用户的网络请求
      ...
      removeUserById(id) {
        ...
        this.$confirm(
          ...
          .then(res => {
          // return this.$message.info('确认了删除')
      ...
      // 监听分配角色对话框的关闭事件，在关闭时将selectedRoleId重置为''
      // 将userInfo重置为{}
      setRoleDialogClosed() {
        this.selectedRoleId = ''
        this.userInfo = {}
