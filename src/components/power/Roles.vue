...
<!--        el-table-column type="expand" el里的展开竖列-->
...
<!--        el-table-column type="index" el里的索引竖列-->
  ...
  export default {
    ...
    methods: {
      ...
      showSetRightDialog(role) {
        // 在弹出分配角色对话框时，将role的 id 保存到 data 中，
        // 以为角色分配权限时使用
        this.roleId = role.id
        // 获取所有权限的数据
        this.axios.get('rights/tree').then(res => {
          ...
          // 获取到的权限数据以数组形式保存到 data 的 rightsList 中
      ...
      // 通过递归的形式，获取角色下的所有三级权限的 Id，
      // 先保存到arr数组中，再保存到数组 defKeys 中
      getLeafKeys(node, arr) {
          // 如果 node 节点不包含 children 属性，则是三级节点,
          // 在return出去之前将找到的三级节点的 Id push到arr里去
          if(!node.children) {
      ...
      allotRights() {
        ...
        // console.log(keys)
        // 以 `,` 分割的权限 ID 列表（获取所有被选中getCheckedKeys()、
        // 叶子节点的key和半选中节点getHalfCheckedKeys()的key,包括 1，2，3级节点
